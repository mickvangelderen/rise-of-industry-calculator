...
use rise_of_industry_calculator::{
    BuildingEfficiency, BuildingIndex, GameData, ModuleIndex, ProductIndex, ProductVec,
    RecipeIndex, TransportKind, BASE_COST_TO_UPKEEP,
};
...
fn main() {
    ...
    for product in tier_sorted_indices {
        ...
        println!(
            "| {tier:4} | {category:20} | {roi:6.2}% | {product:20} | ",
...
fn auto_build(
    ...
    let sales_map: ProductVec<f64> = data
        .products()
        .map(|p| {
            if *p == product_index {
